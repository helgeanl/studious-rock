	...
	while(1){
		...
		if (fifo_status(&queue)){
			...
			if(blocked_id !=-1){
				...
				fifo_rem_string(&queue, char_array);
				MsgReply(blocked_id, 0, char_array, WIDTH);
...
int main(int argc, char *argv[]) {
	...
	pthread_t queue_check_thread;
	pthread_create(&queue_check_thread,NULL,queue_check,NULL);
...
int io_write(resmgr_context_t *ctp, io_write_t *msg, iofunc_ocb_t *ocb)
{
	...
	fifo_add_string(&queue,char_array);
	printf("Queue:\n");
	fifo_print(&queue);
