...
void* threadFunc1(void){
  ...
  print_pri(NULL,"Task Low - acquire semaphore\n");
  busy_wait_ms(30);
  print_pri(NULL,"Task Low - release semaphore\n");
...
void* threadFunc3(){
  ...
  rt_sem_p(&sem,TM_INFINITE);
  //rt_mutex_acquire(&mutex,TM_INFINITE);
  print_pri(NULL,"Task High - acquire semaphore\n");
  busy_wait_ms(20);

  print_pri(NULL,"Task High - release semaphore\n");
  rt_sem_v(&sem);
  //rt_mutex_release(&mutex);
